...
const RequestQuotePopUp: React.FunctionComponent<CustomProps> = (
  props: CustomProps
) => {
  ...
  const onSubmit = async (data: any) => {
    try {
      const response = await api.quickAccess.RequestQuote({
        ...data,
      ...
      if (response.responsecode === 200) {
        router.push(
          `${RouteKeys.QuickAccessThankYou.replace(
            ":quickAccess",
            QuickAccessFormsTypes.requestQuote
          )}`
        );
...
export default connect(mapStateToProps, mapActionsToProps)(RequestQuotePopUp);
