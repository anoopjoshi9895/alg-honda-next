...
import CustomerCarePackageItem from "./CustomerCarePackageItem";
...
const CustomerCarePackageTabPane: React.FunctionComponent<CustomProps> = (
  ...
  const [tabName, setTabName] = React.useState<string | undefined>(
    props.customerCarePackageData && props.customerCarePackageData.length > 0
      ? props.customerCarePackageData[0].productTitle
      : undefined
  );
  ...
  const onCustomerCareSelect = async (
    service: CustomerCareModelCart,
    isChoosen: boolean
  ) => {
    const cart: CartStateModel = cloneDeep(props.cartData);

    cart.customerCare = !isChoosen ? [] : [service];
    addFOCExtraAddonsToConfiguration(
      props.productData,
  ...
  const checkServiceSelected = (id: number) => {
    const cart: CartStateModel = cloneDeep(props.cartData);
    const serviceIndex = cart.customerCare?.findIndex(
      (item) => item.productID === id
    );
        ...
        <div className="d-block w-100">
          <EqualHeight>
            <Slider
              ...
              {...serviceSlider}
              onInit={() =>
                sliderInitialize(props.customerCarePackageData?.length)
              }
              afterChange={(currentSlide) =>
                sliderCurrent(
                  currentSlide,
                  props.customerCarePackageData?.length
                )
              }
            >
              {props.customerCarePackageData?.map((customerCare) => {
                return (
                  <CustomerCarePackageItem
                    customerCarePackage={customerCare}
                    isChoosen={checkServiceSelected(customerCare.productID)}
                    onChoose={onCustomerCareSelect}
                    key={customerCare.productID}
                    campaigns={campaign}
