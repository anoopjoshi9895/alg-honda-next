...
const ServiceItem: React.FunctionComponent<CustomProps> = (
  props: CustomProps
) => {
  ...
  React.useEffect(() => {
    if (props.isChoosen) {
      ...
      if (props.cartData?.serviceDetails.length > 0) {
        const optionSelected =
          props.cartData?.serviceDetails[0]?.planDetails?.productOptionID;

        const option = props?.service?.optionsList.find(
        ...
        setPaymentMode(
          props.cartData?.serviceDetails[0]?.planDetails?.payType === "cash"
            ? "cash"
            : props.cartData?.serviceDetails[0]?.planDetails?.installments.toString()
        );
        if (
          props.cartData?.serviceDetails[0]?.planDetails?.addons?.length > 0
        ) {
          setSelectedAddons(
            props.cartData.serviceDetails[0].planDetails.addons
          );
        }
      }
    } else {
      if (props?.service?.optionsList?.length > 0) {
        const optionSelected = props?.service?.optionsList[0].productOptionID;
  ...
  const getPrice = () => {
    const discountData = getAppliedDiscount(
      ...
      "servicesList",
      {
        productID: props.service.productID,
  ...
  const checkAddonChoosen = (id: number) => {
    ...
    if (props.isChoosen && cart.serviceDetails?.length > 0) {
      const addonCopy = cart.serviceDetails[0]?.planDetails?.addons;
  ...
  const onServiceChoose = (isChoosen: boolean, addonCopy: AddonModelCart[]) => {
    ...
    const addons: AddonModelCart[] = addonsFOC
      ? [...addonCopy, ...addonsFOC]
      : addonCopy;
    const serviceCart: ServicesModelCart = {
      productID: props.service.productID,
      productOptionID: selectedOption?.productOptionID || 0,
      productTitle: props.service.productTitle,
  ...
  const onChangeYear = (optionId: number) => {
    setSelectedAddons([]);
    const option: OptionsModelBV | undefined = props.service?.optionsList?.find(
  ...
  const isIncludedInCampaign = !!props.campaigns?.find((campaign) =>
    campaign?.appliedItems?.find(
      (ai) =>
        ai.productID === props.service?.productID &&
        ai.productType === "servicesList"
    )
  );
  ...
  const isExcludedInCampaign = isProductExcludedFromCampaign(
    "servicesList" as EcomProductType,
    props.service.productID,
    props.cartData!
  );
    ...
    <>
      <div className="col-12">
        <EqualHeightElement name="title">
          <div className="col p-0 insurance-feature-block title">
            <div className="position-relative">
              <Image
                src={props.service.planImage}
                className="img-fluid w-100 img-clip img-cover"
                alt={props.service.productTitle}
                width={327}
                height={190}
              />
        ...
        <div className="font-normal mb-5 d-block border">
              ...
              <div className="col insurance-feature-block">
                <div className="form-group mb-3 pb-1">
                  ...
                  <SelectInput
                    ...
                    data={props.service?.optionsList}
                    default={
                      selectedOption
          ...
          {props.service?.featuresList?.map((feat) => {
            return (
              <EqualHeightElement
              ...
              {extraAddOns && extraAddOns.length > 0 && (
                <div className="col insurance-feature-block">
                  ...
                  {extraAddOns?.map((addon) => {
                    return (
                      <AddonCheckBox
                        key={`${props.service.productID}-${addon.addonID}`}
                        addon={addon}
                        onChange={onChangeAddon}
                        isSelected={checkAddonChoosen(addon.addonID)}
                        insuranceId={props.service.productID}
          ...
          <EqualHeightElement name="agree">
            <div className="row insurance-type-features flex-lg-wrap flex-nowrap justify-content-lg-center no-gutters h-100">
              <div className="col insurance-feature-block">
                <input
                  type="checkbox"
                  id={`terms-${props.service.productID}`}
                  disabled={props.isChoosen}
                  checked={termsAccepted}
                  onChange={() => onTermsAcceptedClick()}
                />
                <label
                  htmlFor={`terms-${props.service.productID}`}
...
export default connect(mapStateToProps, mapActionsToProps)(ServiceItem);
