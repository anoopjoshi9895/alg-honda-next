...
interface CustomProps {
  customerCarePackage: CustomerCareModelBV;
  onChoose: any;
  isChoosen: boolean;
...
const CustomerCarePackageItem: React.FunctionComponent<CustomProps> = (
  props: CustomProps
) => {
  ...
  const onChoose = () => {
    props.onChoose(props.customerCarePackage);
  };
  ...
  React.useEffect(() => {
    if (props.isChoosen) {
      ...
      if (props.cartData?.customerCare.length > 0) {
        const optionSelected =
          props.cartData?.customerCare[0]?.planDetails?.productOptionID;

        const option = props?.customerCarePackage?.optionsList.find(
        ...
        setPaymentMode(
          props.cartData?.customerCare[0]?.planDetails?.payType === "cash"
            ? "cash"
            : props.cartData?.customerCare[0]?.planDetails?.installments.toString()
        );
        if (props.cartData?.customerCare[0]?.planDetails?.addons?.length > 0) {
          setSelectedAddons(props.cartData.customerCare[0].planDetails.addons);
        }
      }
    } else {
      if (props?.customerCarePackage?.optionsList?.length > 0) {
        const optionSelected =
          props?.customerCarePackage?.optionsList[0].productOptionID;
  ...
  const getPrice = () => {
    const discountData = getAppliedDiscount(
      ...
      {
        productID: props.customerCarePackage.productID,
        productOptionID: selectedOption?.productOptionID,
        sortOrder: props.customerCarePackage.sortOrder,
      },
      props.campaigns,
      props.cartData
    );
  ...
  const checkAddonChoosen = (id: number) => {
    ...
    if (props.isChoosen && cart.customerCare?.length > 0) {
      const addonCopy = cart.customerCare[0]?.planDetails?.addons;
  ...
  const onServiceChoose = (isChoosen: boolean, addonCopy: AddonModelCart[]) => {
    ...
    const addons: AddonModelCart[] = addonsFOC
      ? [...addonCopy, ...addonsFOC]
      : addonCopy;
    const serviceCart: CustomerCareModelCart = {
      productID: props.customerCarePackage.productID,
      productOptionID: selectedOption?.productOptionID || 0,
      productTitle: props.customerCarePackage.productTitle,
      salesPrice: originalAmount,
      cartItemID: 0,
      menuCode: "",
      offerPrice: offerAmount,
      sortOrder: props.customerCarePackage.sortOrder || 0,
      planDetails: {
  ...
  const onChangeYear = (optionId: number) => {
    ...
    const option: OptionsModelBV | undefined =
      props.customerCarePackage?.optionsList?.find(
  ...
  const isIncludedInCampaign = !!props.campaigns?.find((campaign) =>
    campaign?.appliedItems?.find(
      (ai) =>
        ai.productID === props.customerCarePackage?.productID &&
        ai.productType === "customerCarePackages"
    )
  );
  ...
  const isExcludedInCampaign = isProductExcludedFromCampaign(
    "customerCarePackages" as EcomProductType,
    props.customerCarePackage.productID,
    props.cartData!
  );
    ...
    <>
      <div className="col-12">
        <EqualHeightElement name="title">
          <div className="col p-0 insurance-feature-block title">
            <div className="position-relative">
              <Image
                src={props.customerCarePackage.planImage}
                className="img-fluid w-100 img-clip img-cover"
                alt={props.customerCarePackage.productTitle}
                width={327}
                height={190}
              />
              {/* <div className="d-flex flex-column flex-wrap align-items-center justify-content-center position-absolute top-0 left-0 w-100 text-center content p-2 pt-4 insurance-type-head">
                <h6 className="font-weight-bold text-uppercase insurance-title">
                  {props.customerCarePackage.productTitle}
                </h6>
              </div> */}
            </div>
          </div>
        ...
        <div className="font-normal mb-5 d-block border">
              ...
              <div className="col insurance-feature-block">
                <div className="form-group mb-3 pb-1">
                  ...
                  <SelectInput
                    ...
                    data={props.customerCarePackage?.optionsList}
                    default={
                      selectedOption
          ...
          {props.customerCarePackage?.featuresList?.map((feat) => {
            return (
              <EqualHeightElement
              ...
              {extraAddOns && extraAddOns.length > 0 && (
                <div className="col insurance-feature-block">
                  ...
                  {extraAddOns?.map((addon) => {
                    return (
                      <AddonCheckBox
                        key={`${props.customerCarePackage.productID}-${addon.addonID}`}
                        addon={addon}
                        onChange={onChangeAddon}
                        isSelected={checkAddonChoosen(addon.addonID)}
                        insuranceId={props.customerCarePackage.productID}
          ...
          <EqualHeightElement name="agree">
            <div className="row insurance-type-features flex-lg-wrap flex-nowrap justify-content-lg-center no-gutters h-100">
              <div className="col insurance-feature-block">
                <input
                  type="checkbox"
                  id={`terms-${props.customerCarePackage.productID}`}
                  disabled={props.isChoosen}
                  checked={termsAccepted || props.isChoosen}
                  onChange={() => onTermsAcceptedClick()}
                />
                <label
                  htmlFor={`terms-${props.customerCarePackage.productID}`}
