...
export const getStaticProps = async ({ locale, locales, params }: any) => {
  ...
  var motorcycleDetail = await loadMotorcycleDetail(language, slug);
  const footerData = await loadFooterData(locale);

  if (!motorcycleDetail || !footerData) {
    return {
      notFound: true,
      props: {},
      revalidate: 1,
...
export const getStaticPaths = async ({ locales, locale, query }: any) => {
  const language = locale;
  const type = "land";
  var motorcycleSlug = await loadMotorCylceMenuLandingPageStaticpath("en", type);
  const paths: any = [];
  motorcycleSlug?.slider?.forEach((p) => {
    paths.push({ params: { slug: p.slug.toString() }, locale: "en" });
  });
